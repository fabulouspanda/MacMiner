...
@implementation taskThreeWrapper
...
- (id)initWithCommandPath:(NSString *)pathToCommand
			  ...
			  environment:(NSDictionary *)env
				 delegate:(id <taskThreeWrapperDelegate>)aDelegate;
{
	self = [super init];
	if (self)
	{
...
- (void)startTask
{
	...
	if ([(id)_taskDelegate respondsToSelector:@selector(taskThreeWrapperWillStartTask:)])
	{
		[_taskDelegate taskThreeWrapperWillStartTask:self];
	}
...
{
	if ([data length] && [(id)_taskDelegate respondsToSelector:@selector(taskThreeWrapper:didProduceOutput:)])
	{
		NSString *outputString = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
		
		[_taskDelegate taskThreeWrapper:self didProduceOutput:outputString];
	}
}
...
- (void)stopTask
{
	...
	if ([(id)_taskDelegate respondsToSelector:@selector(taskWrapper:didFinishTaskWithStatus:)])
	{
		...
		[_taskDelegate taskThreeWrapper:self didFinishTaskWithStatus:taskStatus];
	};
