...
- (void)startTask
{
	...
	if ([(id)_taskDelegate respondsToSelector:@selector(taskTwoWrapperWillStartTask:)])
	{
		[_taskDelegate taskTwoWrapperWillStartTask:self];
	}
...
{
	if ([data length] && [(id)_taskDelegate respondsToSelector:@selector(taskTwoWrapper:didProduceOutput:)])
	{
		NSString *outputString = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
		
		[_taskDelegate taskTwoWrapper:self didProduceOutput:outputString];
	}
}
